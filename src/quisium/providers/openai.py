...
def _build_openai_tool_schema(tool_call: ToolCall) -> Dict[str, Any]:
    schema = tool_call.schema
    # When schema contains a nested "parameters" key the full function spec was stored
    # (e.g. via chat_with_tool_definitions). Otherwise treat the whole dict as parameters.
...
class OpenAIProvider(BaseProvider):
    ...
    def chat_with_tool_definitions(
        ...
        **kwargs: Any,
        ...
        # Convert OpenAI-format dicts to ToolCall so the tool guard can validate names.
        # We store the full function spec in schema so _build_openai_tool_schema can
        # reconstruct the complete API payload (description + parameters) when
        # base.py forwards tools= to _call_model.
        guard_tools = [
            ToolCall(
                name=td.get("function", {}).get("name", ""),
                args={},
                schema=td.get("function", {}),
            )
